...
**Then make it safe to ship.** Rebuild the dispatch table from a recorded
...
the hackathon get made: the roofline plotted from `runs/benchmark.csv` against
...
`runs/ablation/<arm>/`, and a second, unrelated kernel run through the same
