# Transformer Layer -- Phase 2: Make It Fast, With Evidence
...
**Development card: NVIDIA RTX 6000 Ada Generation (AD102, sm_89, 142 SMs,
48 GB GDDR6 ECC, ~960 GB/s).** Rented by the hour. This is the card every row
in `runs/benchmark.csv` was measured on; `./scripts/check_gpu.sh` prints the
board name, driver and torch build the box actually has, and that printout --
not this paragraph -- is what a recorded number is judged against.

The 48 GB matters for the shape grid: it is twice a 4090's, and the stress
geometry (`S=100000`) is bounded by memory, not by time. A second card is used
late for a cross-hardware check.
...
- Large L2 (96 MB on this board): at these shapes the whole working set may be
  resident, which changes what "memory-bound" means here. Confirm the figure
  from the device properties rather than from this line.

Every number above is a spec sheet. The roofline uses ceilings **measured** on
this board by `scripts/measure_ceilings.py` -- a large GEMM and a large copy --
and never these.
...
**Profile before optimizing.** Collect an NCU report for the default shape and
...
all. At 1024 tokens and ~90 launches per forward, do not assume it is the
matmuls.

**Directions worth ranking** (rank them yourself, with reasons; this is a
starting list, not an ordering):

- CUDA Graph capture, to collapse per-launch overhead.
- Eliminating the four whole-tensor materializations per layer.
- Fusing LayerNorm with the residual add.
- Fusing the FFN's GELU into the surrounding GEMMs.
- Attention backend selection per dtype and mask combination.
- `torch.compile` on our candidate rather than the baseline, and where its
  guards force recompilation.
- Weight layout: pre-packing fused QKV, transposition, alignment.
- Occupancy, register pressure and tail effects at these small shapes.
...
**Guard against the two ways this goes wrong.** Tuning against the default shape
until everything else regresses -- run the full development set, not one case.
And reporting a best-of run -- the median is the metric, p90 is the honesty
check.
