...
**Both fixes below need root on the host.** In a rented container you have
neither that nor any way to grant the container the capability, and no
in-container workaround exists -- see
[`10-no-counter-fallback.md`](10-no-counter-fallback.md) for how to tell which
gate shut, and for the `nsys` / `torch.profiler` path that still works without
counters. Check this before trying either of the following:

```bash
grep RmProfilingAdminOnly /proc/driver/nvidia/params   # 1 = host driver refuses it
grep CapEff /proc/self/status                          # needs CAP_SYS_ADMIN
```

Two solutions, on a host you control:
