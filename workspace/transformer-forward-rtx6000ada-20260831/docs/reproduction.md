...
**NVIDIA RTX 6000 Ada Generation for development, rented by the hour. A second
card late, for the cross-hardware check.**

The plan called for an RTX 4090; what was actually available and rented was an
RTX 6000 Ada Generation. Same die (AD102), same compute capability (sm_89), so
every architectural claim in `prompts/_shared.md` survives the substitution --
but it has 142 SMs to the 4090's 128, and 48 GB of GDDR6 to the 4090's 24 GB.
The VRAM is the difference that changed the work; see the stress-shape note
below. Every row in `runs/benchmark.csv` records the board name it was measured
on, and that field, not this paragraph, is the authority.
...
On an Ada workstation card the same work is a few hundred microseconds -- the
same order as the overhead. The smaller card makes the problem *more*
interesting, not less.

Approximate on-demand rates as of late August 2026 -- verify before booking:
RTX 4090 around $0.28-0.50/hr, RTX 6000 Ada around $0.70-0.80/hr, A100 80GB
around $1/hr, H100 from roughly $1/hr spot. Forty hours of Ada development plus
a few hours of H100 validation is tens of dollars, which is why none of this
needs a purchase.

One caveat from the real grid: appendix shape #14 (`S=100000, d=1024, B=32`)
needs ~38 GB for q/k/v alone at fp32. That does not fit the 24 GB card the plan
assumed, which is why the plan sent the stress shape to the 80 GB-class
validation hours. The 48 GB actually rented changes that: the sweep runs on the
development card up to its own ceiling, and shape #14 is reachable off-script
rather than only in validation hours. It is still not an official pass -- the
evaluator's own eager reference cannot produce a comparison at that size on any
hardware. See `docs/benchmark-anatomy.md` §9.
