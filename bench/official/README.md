...
Point the harness at a different copy with:
...
organizers will run, so this repository targets the *stricter* pair — a
candidate that clears `0.001 / 0.01` clears both. `odyssey` defaults to the
strict pair; pass `--atol/--rtol` to reproduce the loose one.
