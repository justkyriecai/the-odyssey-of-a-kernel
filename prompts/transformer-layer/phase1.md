...
**Establish the opponent before you build.** Run the baseline ladder first:

```bash
python -m odyssey ladder --shapes dev --rungs L0 L1 L2
```

L0 is eager with TF32 already on. L2 is `torch.compile` at `max-autotune`, one
flag away in the official script. That is the number anyone in the room will ask
about. Do not write a kernel before you know it.
...
**Verify.** Both gates, every time:

```bash
python -m odyssey bench <candidate> --shapes dev
python -m odyssey official <candidate> --shapes dev --case center-padded -- --atol 0.001 --rtol 0.01
```
