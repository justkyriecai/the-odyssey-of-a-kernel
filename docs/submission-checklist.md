...
- [ ] Repository is **public**, and the clone-and-run path in `README.md` works
      from a fresh checkout (`./scripts/setup_env.sh && ./scripts/smoke.sh`).
- [ ] `git ls-files | grep -E "^(ref|runs|workspaces)/"` returns nothing —
      reference material and generated evidence stay out of the tree.

## Numbers

- [ ] Final hardware, final run: `odyssey calibrate --shapes official` re-run on
      the card the report quotes, so `runs/dispatch_table.json` was built where
      the numbers were.
...
- [ ] `odyssey peak` run on the final card — the roofline uses **measured**
      ceilings, not `DEVICE_PEAKS` spec-sheet entries.
- [ ] `odyssey roofline` regenerated from the final `benchmark.csv`.
- [ ] `odyssey ablation --plot` regenerated after all three arms ran, with wall
      clock recorded in each arm's `meta.json`.
- [ ] `odyssey dag --out runs/search.dot && dot -Tsvg` — the search DAG in the
