...
One row per (case, candidate), from `runs/benchmark.csv`. `abs_budget_spent` is
`max_abs_error / atol`: below 1.0 means margin remains, and how much.

```bash
python -m odyssey bench all --shapes official
python - <<'PY'
import csv
rows = list(csv.DictReader(open("runs/benchmark.csv")))
print(f"{'case':<16}{'candidate':<14}{'max_abs':>10}{'spent':>8}{'max_rel':>10}{'ok':>5}")
for r in rows:
    if not r["max_abs_error"]:
        continue
    print(f"{r['case']:<16}{r['candidate']:<14}{float(r['max_abs_error']):>10.2e}"
          f"{float(r['abs_budget_spent'] or 0):>8.2f}{float(r['max_relative_error']):>10.2e}"
          f"{'Y' if r['accuracy_passed']=='True' else 'N':>5}")
PY
```
