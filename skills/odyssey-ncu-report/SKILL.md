---
name: odyssey-ncu-report
description: Profile CUDA kernels with Nsight Compute on B200 / sm_100, and fall back to nsys timelines or torch.profiler when hardware counters are denied. Use when the user asks to profile a kernel, analyze its performance, diagnose bottlenecks, read an ncu report, or write an optimization plan — including when ncu fails with ERR_NVGPUCTRPERM on a rented pod, and variants in Chinese ("profile 一下", "为什么慢", "ncu 报告", "ncu 没权限").
---
...
0b. **Check that counters are actually available**, before building anything around them. On a rented container they very often are not:

    ```bash
    ncu --metrics sm__cycles_elapsed.avg python -c "import torch; torch.zeros(8, device='cuda')" 2>&1 | tail -3
    ```

    `ERR_NVGPUCTRPERM` means the rest of this quickstart cannot run as written. Do **not** go looking for a workaround inside the container — both gates that cause it live outside one. Switch to [`reference/10-no-counter-fallback.md`](reference/10-no-counter-fallback.md), which diagnoses whose problem it is and runs the same investigation on `nsys` timelines and `torch.profiler`. That path still answers "launch-bound, memory-bound or compute-bound?"; it does not give occupancy, stalls or a measured roofline, and the report has to say so.
...
| [`reference/10-no-counter-fallback.md`](reference/10-no-counter-fallback.md) | **When `ncu` is denied.** Which gate shut and whose it is, the `nsys` / `torch.profiler` path that still works, what it answers and what it cannot |
...
6. **A denied counter is a fact about the platform, not a step to retry.** `RmProfilingAdminOnly: 1` on the host and a container without `CAP_SYS_ADMIN` are both set outside the container; a different card or a fresh pod from the same provider behaves identically. Diagnose it once, record it, and move to the fallback path — see [`reference/10-no-counter-fallback.md`](reference/10-no-counter-fallback.md).

7. **Name the instrument next to the number.** A report built on `nsys` timelines is a legitimate report; one that reads like an `ncu` report but was not measured with counters is not. State at the top which path produced it, and write "not measured" where a counter number would have gone.
