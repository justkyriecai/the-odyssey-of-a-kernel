...
@register(
    "graph-safe",
    description="fused-safe captured into a CUDA Graph. One replay instead of ~90 launches.",
    requires=("cuda", "cuda-graphs"),
    tags=("phase2",),
)
def build_safe(*, official: Any, config: Any, case: Any) -> Any:
    return cuda_graph_class(fused_attention_class(official, FP32_SOFTMAX))(config)


@register(
    "graph-sdpa",
    description="fused-sdpa captured into a CUDA Graph.",
    requires=("cuda", "cuda-graphs"),
    tags=("phase2",),
)
def build_sdpa(*, official: Any, config: Any, case: Any) -> Any:
    return cuda_graph_class(fused_attention_class(official, SDPA))(config)
