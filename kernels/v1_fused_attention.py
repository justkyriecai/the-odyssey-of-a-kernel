...
`odyssey calibrate` measures both on the card in front of you and admits
...
@register(
    "fused-safe",
    description="Fused QKV, no per-head copies, baseline-exact fp32 softmax. Zero error by construction.",
    tags=("phase1",),
)
def build_safe(*, official: Any, config: Any, case: Any) -> Any:
    return fused_attention_class(official, FP32_SOFTMAX)(config)


@register(
    "fused-sdpa",
    description="Fused QKV, no per-head copies, scaled_dot_product_attention. Faster where the backend keeps fp32 accumulation.",
    tags=("phase1",),
)
def build_sdpa(*, official: Any, config: Any, case: Any) -> Any:
    return fused_attention_class(official, SDPA)(config)
